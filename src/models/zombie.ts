...
import { BLUE_GREEN_MAT, BROWN_MAT, LIGHT_STONE_MAT } from '../material';
...
export class ZombieMesh {
  ...
  private createHead(): void {
    const head = new THREE.Mesh(faceGeo, BLUE_GREEN_MAT);
    head.position.y = 2;
    this.body.add(head);
  }
  ...
  private createLegs(): void {
    ...
    const leftFoot = new THREE.Mesh(footGeo, BLUE_GREEN_MAT);
    ...
    const rightFoot = new THREE.Mesh(footGeo, BLUE_GREEN_MAT);
  ...
  private createArms(): [THREE.Object3D, THREE.Object3D] {
    ...
    const leftHand = new THREE.Mesh(lowerArmGeo, BLUE_GREEN_MAT);
    ...
    const rightHand =new THREE.Mesh(lowerArmGeo, BLUE_GREEN_MAT);
